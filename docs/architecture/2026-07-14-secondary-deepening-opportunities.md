...
| Homepage composition outside the deck preview | `src/pages/index.astro` is 982 lines and combines post selection, date normalization, tag-cloud data, structured data, hero configuration, recent posts, and multiple non-deck feature sections. | Medium | Build-time content queries and presentation decisions share the page module, making homepage changes require broad context. | Use a focused interface-design pass. Build a typed homepage view model in a server-side module, then let non-preview sections consume narrow data props; avoid a generic “section engine.” |
