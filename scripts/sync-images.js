...
function sharpToWebp(sourcePath, destPath, ms = 15000) {
  return Promise.race([
    sharp(sourcePath).webp({ quality: 85 }).toFile(destPath),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`sharp timed out after ${ms}ms`)), ms)
    ),
  ]);
}
...
const IMAGE_SYNC_CONFIGS = [
  ...
  {
    source: 'src/content/bin/pages/attachments',
    target: 'public/pages/attachments',
    name: 'pages'
  },
  {
    source: 'src/content/bin/projects/attachments',
    target: 'public/projects/attachments',
    name: 'projects'
  },
  {
    source: 'src/content/bin/docs/attachments',
    target: 'public/docs/attachments',
    name: 'docs'
  },
  {
    source: 'src/content/bin/special/attachments',
...
async function findImageFiles(dir, relativePath = '') {
  ...
  try {
    ...
    // Build a Set of original image basenames (without extension) for O(1) webp lookup
    const originalBasenames = new Set();
    for (const i of items) {
      if (/\.(jpg|jpeg|png|gif|bmp|tiff|tif)$/i.test(i)) {
        originalBasenames.add(i.replace(/\.(jpg|jpeg|png|gif|bmp|tiff|tif)$/i, ''));
      }
    }
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (item.toLowerCase().endsWith('.webp')) {
          ...
          if (originalBasenames.has(originalName)) {
            continue;
          }
        }
        imageFiles.push({
          ...
          if (needsUpdate) {
            ...
            if (/\.(jpg|jpeg|png|gif|bmp|tiff|tif)$/i.test(imageFile.relativePath)) {
              try {
                ...
                await sharpToWebp(imageFile.sourcePath, webpPath);
                totalSynced++;
              } catch (error) {
                log.warn(`⚠️  Skipping unsupported image ${imageFile.relativePath}:`, error.message);
              }
            } else {
